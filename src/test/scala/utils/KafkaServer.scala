...
object KafkaServer {
  ...
  def apply[F[_] : Async : LoggerFactory]: Resource[F, KafkaServer.Running[F]] =
    Resource.eval(LoggerFactory[F].create).flatMap(implicit logger => serverResource[F])

  enum State {
    case Starting, Started, Stopping, Stopped
  }

  private def serverResource[F[_] : Logger](using F: Async[F]): Resource[F, KafkaServer.Running[F]] =
    for {
      kafkaPort      <- RandomPort[F].toResource
      controllerPort <- RandomPort[F].toResource
      clusterId      <- F.blocking(Uuid.randomUuid().toString).toResource
      _              <- debug"Kafka port $kafkaPort - Controller port $controllerPort - Cluster ID $clusterId".toResource
      logDir         <- F.blocking(Files.createTempDirectory("embedded-kafka")).toResource
      _              <- debug"Created log directory ${logDir.toAbsolutePath.toString}".toResource
      config         <- F.blocking(JKafkaConfig(properties(logDir, kafkaPort, controllerPort).widen[Object].asJava)).toResource
      _              <- formatLogDirectory(clusterId, config).toResource
      time           <- F.delay(Time.SYSTEM).toResource
      server         <- resource(config, time, kafkaPort, logDir)
    } yield server

  private def resource[F[_] : Logger](
      config: JKafkaConfig,
      time: Time,
      port: Int,
      logDir: Path
  )(using F: Async[F]): Resource[F, KafkaServer.Running[F]] =
    Resource.make {
      for {
        state  <- Ref.of(State.Starting)
        server <- F.blocking(KafkaRaftServer(config, time))
        _      <- F.blocking(server.startup())
        _      <- debug"Server started"
        _      <- state.set(State.Started)
      } yield new KafkaServer.Running[F] {
        override def config: KafkaConfig = KafkaConfig(protocol = "PLAINTEXT", host = "localhost", kafkaPort = port)
        ...
        override def stop: F[Unit] = {
          ...
          for {
            ...
            _            <- shutdown.guarantee(state.set(State.Stopped))
          } yield ()
        }
      }
    } { running =>
      ...
      running.stop.guarantee(deleteLogDir)
    }

  private def properties(logDir: Path, kafkaPort: Int, controllerPort: Int): Map[String, String] = {
    val plaintextListener: String  = s"${SecurityProtocol.PLAINTEXT}://localhost:$kafkaPort"
    val controllerListener: String = s"CONTROLLER://localhost:$controllerPort"
    ...
    Map[String, String](
      ...
      QuorumConfig.QUORUM_VOTERS_CONFIG                                  -> s"0@localhost:$controllerPort",
      ...
      SocketServerConfigs.ADVERTISED_LISTENERS_CONFIG                    -> plaintextListener,
      SocketServerConfigs.LISTENERS_CONFIG                               -> s"$plaintextListener,$controllerListener",
  ...
  private def formatLogDirectory[F[_]](clusterId: String, kafkaConfig: JKafkaConfig)(using F: Sync[F]): F[Int] =
    F.blocking(
      StorageTool.formatCommand(
        stream = PrintStream(OutputStream.nullOutputStream()),
        directories = kafkaConfig.logDirs.toSeq,
        metaProperties = StorageTool.buildMetadataProperties(clusterId, kafkaConfig),
